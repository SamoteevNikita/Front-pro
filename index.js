...
function parsedBalance(balance) {
    let parsedBalance = parseFloat(balance.replace(/[$,]/g, ""));
    return parsedBalance;
}

for (let index in users) {
    if (parsedBalance(users[index].balance) > 2000) {
        phones.push(users[index].phone); 
    }
    sum += parsedBalance(users[index].balance); 
}

alert(`Phones of users with balance > 2000: ${phones}`);
alert(`Total balance of all users: ${sum.toFixed(2)}`);